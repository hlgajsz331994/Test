...
public class KettleDatabaseRepositoryConnectionDelegate extends KettleDatabaseRepositoryBaseDelegate {
  ...
  public synchronized LongObjectId getNextID( String tableName, String fieldName ) throws KettleException {
    ...
    RowMetaAndData r = database.getOneRow("SELECT NEXTVAL('"+tableName+"');");
    ...
    if ( r != null ) {
      ...
      if ( id == null || id == 0) {
        if ( log.isDebug() ) {
          log.logDebug( "error happened -->  next id for   table:  " + tableName + "  idfield:  " + fieldName);
        }
        retval = new LongObjectId( 1 );
      } else {
        if ( log.isDebug() ) {
          log.logDebug("select nextval('" + tableName + "'); --> " + id);
        }
        retval = new LongObjectId( id.longValue());
      }
    }
    return retval;
  ...
  private synchronized LongObjectId getNextLogID( String tablename, String idfield ) throws KettleException {
    LongObjectId retval = null;
    RowMetaAndData r = database.getOneRow("select nextval('"+tablename+"');");
    ...
    if ( r != null ) {
      ...
      if ( id == null || id == 0) {
        if ( log.isDebug() ) {
          log.logDebug( "error happened -->  next id for   table:  " + tablename + "  idfield:  " + idfield);
        }
        retval = new LongObjectId( 1 );
      } else {
        if ( log.isDebug() ) {
  ...
  public synchronized ObjectId getNextLogID() throws KettleException {
    return getNextID(KettleDatabaseRepository.TABLE_R_REPOSITORY_LOG,
