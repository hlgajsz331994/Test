  ...
  public void doGet( HttpServletRequest request, HttpServletResponse response ) throws ServletException,
    ...
    if ( useXML ) {
      ...
      if ( serverConfig != null ) {
        ...
        boolean repoIsClosed = false;
        try {
          Repository repository = serverConfig.getRepository();
          if(repository instanceof KettleDatabaseRepository){
            KettleDatabaseRepository repository_db = (KettleDatabaseRepository)repository;
            repository_db.init(repository.getRepositoryMeta());
            repository_db.connectionDelegate.disconnect();
            repository_db.connectionDelegate.connect();
            Connection connection = repository_db.connectionDelegate.getDatabase().getConnection();
            connection.prepareStatement("SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = 'jingwei_repo' ").executeQuery();
          }
        } catch (Exception e) {
          e.printStackTrace();
          repoIsClosed=true;
        ...
        try {
          ...
          if(repoIsClosed) {
            throw new KettleException(repositoryName == null ? "": repositoryName + " repo is closed!");
          }
        } catch ( Exception e ) {
